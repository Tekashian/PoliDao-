...
describe("UUPS Proxy – CoreUUPSMock upgrade flow", function () {
  it("deploys V1 behind ERC1967Proxy, then upgrades to V2 preserving state", async () => {
    ...
    // Use existing Storage from fixture-like deploy or deploy a simple MockStorage if needed.
    // Here we reuse real Storage for realism.
    ...
    await expect(coreV1.connect(owner).upgradeTo(await v2.getAddress())).to.not.be.reverted;
